...
# set the pins for the ultrasonic senesor (Triger Pin Out & Echo Pin In)
trigPin = 16
echoPin = 18
...
# function to call the ultrasonic senesor 
def ulrasonicSensor():
    
    # send the signals from the triger pin
    # set the trigPin by 0 for 2 Micro Second (E-6 refeare to Micro Second)
    GPIO.output(trigPin,0)
    time.sleep(2E-6)
    # set the trigPin by 1 for 10 Micro Second (E-6 refeare to Micro Second)
    GPIO.output(trigPin,1)
    time.sleep(10E-6)
    # set the trigPin by 0 again
    GPIO.output(trigPin,0)
    
    # count the time between the sending and reciving the signal
...
# function to call the PIR senesor 
def pirSensor():
...
# in this function it will call all sensors fnuctions and handle the logic calling 
def main():
    try:        
        while True:
            distance = ulrasonicSensor()
            motion = pirSensor()
            opened = magnetSensor()
